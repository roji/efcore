...
public sealed partial class SelectExpression : TableExpressionBase
{
        ...
        : base(null)
    {
        switch (entityType.GetMappingStrategy())
        {
            case RelationalAnnotationNames.TptMappingStrategy:
            {
                ...
                var tableMap = new Dictionary<ITableBase, TableExpressionBase>();
                ...
                foreach (var baseType in entityType.GetAllBaseTypesInclusive())
                {
                    ...
                    tableMap.Add(table, tableExpression);
                    ...
                    else
                    {
                        var innerColumns = keyProperties.Select(
                            p => CreateColumnExpression(p, table, tableExpression, nullable: false));
                ...
                foreach (var derivedType in entityType.GetDerivedTypes())
                {
                    ...
                    var keyColumns = keyProperties.Select(p => CreateColumnExpression(p, table, tableExpression, nullable: true))
                        .ToArray();
            ...
            case RelationalAnnotationNames.TpcMappingStrategy:
            {
                ...
                if (entityTypes.Length == 1)
                {
                    ...
                    var tableExpression = new TableExpression(table);
                    var tableMap = new Dictionary<ITableBase, TableExpressionBase> { [table] = tableExpression };
                    AddTable(tableExpression);
                ...
                else
                {
                    ...
                    for (var i = 0; i < entityTypes.Length; i++)
                    {
                        ...
                        for (var j = 0; j < properties.Length; j++)
                        {
                            ...
                            var projection = declaringEntityType.IsAssignableFrom(concreteEntityType)
                                ? CreateColumnExpression(property, table, tableExpression, declaringEntityType != entityType)
                    ...
                    for (var i = 0; i < properties.Length; i++)
                    {
                        columns[properties[i]] = CreateColumnExpression(firstSelectExpression._projection[i], tpcTables);
                    }
                    ...
                    var discriminatorColumn = CreateColumnExpression(firstSelectExpression._projection[^1], tpcTables);
                    _tpcDiscriminatorValues[tpcTables] = (discriminatorColumn, discriminatorValues);
                    var tableMap = tables.ToDictionary(t => t, _ => (TableExpressionBase)tpcTables);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        foreach (var mapping in mappings)
                        {
                            ...
                            var tableExpression = new TableExpression(table);
                            tableMap[table] = tableExpression;
                            ...
                            if (_tables.Count == 0)
                            {
                                ...
                                foreach (var property in keyProperties)
                                {
                                    var columnExpression = CreateColumnExpression(
                                        property, table, tableExpression, nullable: false);
        ...
        void GenerateNonHierarchyNonSplittingEntityType(ITableBase table, TableExpressionBase tableExpression)
        {
            ...
            var projection = new StructuralTypeProjectionExpression(
                entityType,
                propertyExpressions,
                new Dictionary<ITableBase, TableExpressionBase> { [table] = tableExpression });

            AddJsonNavigationBindings(entityType, projection, propertyExpressions, tableExpression);
            _projectionMapping[new ProjectionMember()] = projection;
        ...
        static ITableBase GetTableBaseFiltered(IEntityType entityType, Dictionary<ITableBase, TableExpressionBase> existingTables)
        ...
        : base(null)
    {
        ...
        var tableMap = new Dictionary<ITableBase, TableExpressionBase> { [table] = tableExpressionBase };
        ...
        AddJsonNavigationBindings(entityType, projection, propertyExpressions, tableExpressionBase);
        _projectionMapping[new ProjectionMember()] = projection;
                ...
                switch (value)
                {
                    ...
                    case ShapedQueryExpression { ResultCardinality: ResultCardinality.Enumerable } shapedQueryExpression:
                    {
                        ...
                        if (querySplittingBehavior == QuerySplittingBehavior.SplitQuery)
                        {
                            ...
                            innerSelectExpression =
                                (SelectExpression)new ColumnTableReferenceUpdater(this, outerSelectExpression).Visit(innerSelectExpression);
                                        ...
                                        foreach (var ordering in orderingsToBeErased)
                                        {
                                            innerOrderingExpressions.Add(
                                                new OrderingExpression(
                                                    subquery.GenerateOuterColumn(collectionJoinedInnerTable, ordering.Expression),
            ...
            void GetOrderingsFromInnerTable(TableExpressionBase tableExpressionBase, List<OrderingExpression> orderings)
            {
                        ...
                        orderings.Add(
                            new OrderingExpression(
                                joinedSubquery.GenerateOuterColumn(
                                    tableExpressionBase,
                                    rowNumberSubquery.GenerateOuterColumn(rowNumberSubquery, partition)),
                                ascending: true));
                    }
                    ...
                    foreach (var ordering in rowNumberExpression.Orderings)
                    {
                        orderings.Add(
                            new OrderingExpression(
                                joinedSubquery.GenerateOuterColumn(
                                    tableExpressionBase,
                                    rowNumberSubquery.GenerateOuterColumn(rowNumberSubquery, ordering.Expression)),
                                ordering.IsAscending));
                    }
                }
                ...
                else if (tableExpressionBase is SelectExpression { Orderings.Count: > 0 } collectionSelectExpression)
                {
                    foreach (var ordering in collectionSelectExpression.Orderings)
                    {
                        orderings.Add(
                            new OrderingExpression(
                                collectionSelectExpression.GenerateOuterColumn(tableExpressionBase, ordering.Expression),
                                ordering.IsAscending));
                    }
                }
            }
        ...
        if (groupByTerms.Any(e => e is not ColumnExpression))
        {
            ...
            for (var i = 0; i < groupByTerms.Count; i++)
            {
                ...
                var newItem = subquery._projection.Any(e => e.Expression.Equals(item))
                    ...
                    : subquery.GenerateOuterColumn(subquery, item, groupByAliases[i] ?? "Key");
    ...
    private void ApplySetOperation(SetOperationType setOperationType, SelectExpression select2, bool distinct)
    {
        ...
        var select1 = new SelectExpression(
            alias: null, new List<ProjectionExpression>(), _tables.ToList(), _groupBy.ToList(), _orderings.ToList(),
            GetAnnotations())
        {
                     ...
                     (kv1, kv2) => (kv1.Key, Value1: kv1.Value, Value2: kv2.Value)))
        {
            ...
            var outerProjection = CreateColumnExpression(innerProjection1, setExpression);
            ...
            StructuralTypeProjectionExpression projection2)
        {
                ...
                StructuralTypeProjectionExpression nestedProjection2)
            {
                ...
                foreach (var property in GetAllPropertiesInHierarchy(type))
                {
                    ...
                    var outerColumn = CreateColumnExpression(innerProjection, setExpression);
            ...
            var tableMap = projection1.TableMap.ToDictionary(kvp => kvp.Key, kvp => (TableExpressionBase)setExpression);
                ...
                && projection2.DiscriminatorExpression != null)
            {
                ...
                discriminatorExpression = CreateColumnExpression(innerProjection, setExpression);
            }
    ...
    public void ApplyDefaultIfEmpty(ISqlExpressionFactory sqlExpressionFactory)
    {
        ...
        _tables.Clear();
        AddTable(dummySelectExpression);
        ...
        ISqlExpressionFactory sqlExpressionFactory)
    {
        ...
        {
            var propertyExpressions = new Dictionary<IProperty, ColumnExpression>();
            var tableExpressionBase = identifyingColumn.Table.UnwrapJoin();
            if (tableExpressionBase is SelectExpression subquery)
            {
                ...
                {
                    var outerColumn = subquery.GenerateOuterColumn(identifyingColumn.Table, columnExpression);
            ...
            var tableMap = new Dictionary<ITableBase, TableExpressionBase>();
            var keyProperties = entityType.FindPrimaryKey()!.Properties;
            TableExpressionBase mainTableExpression;
            // TableReference mainTableReferenceExpression;
            // TableReference tableReferenceExpression;
            if (tableExpressionBase is TableExpression)
            {
                ...
                if (matchingTableIndex != -1)
                {
                    ...
                    mainTableExpression = selectExpression._tables[tableIndex];
                    tableMap[dependentMainTable] = mainTableExpression;
                    ...
                    if (dependentTables.Count > 1)
                    {
                        ...
                        {
                            var columnExpression = CreateColumnExpression(
                                property, dependentMainTable.FindColumn(property)!, mainTableExpression, pkColumnsNullable);
                        ...
                        for (var i = 1; i < dependentTables.Count; i++)
                        {
                            ...
                            if (matchingTableIndex != -1)
                            {
                                ...
                                tableMap[table] = selectExpression._tables[baseTableIndex + matchingTableIndex];
                            }
                            else
                            {
                                ...
                                // tableReferenceExpression = new TableReference(selectExpression, tableExpression.Alias!);
                                tableMap[table] = tableExpression;
                                ...
                                var joinExpression = new LeftJoinExpression(tableExpression, joinPredicate, prunable: true);
                                selectExpression.AddTable(joinExpression);
                            }
                        }
                    }
            ...
            var ownerTableExpression = selectExpression._tables[baseTableIndex];
            foreach (var property in navigation.ForeignKey.PrincipalKey.Properties)
            {
                var columnBase = principalTables.Select(e => e.FindColumn(property)).First(e => e != null)!;
                var columnExpression = CreateColumnExpression(property, columnBase, ownerTableExpression, pkColumnsNullable);
            ...
            var outerJoinPredicate = ownerJoinColumns
                .Zip(
                    navigation.ForeignKey.Properties
                        .Select(p => CreateColumnExpression(p, dependentMainTable, ownedTable, nullable: false)))
                .Select(i => sqlExpressionFactory.Equal(i.First, i.Second))
                .Aggregate(sqlExpressionFactory.AndAlso);
            var joinedTable = new LeftJoinExpression(ownedTable, outerJoinPredicate);
            tableMap[dependentMainTable] = ownedTable;
            selectExpression.AddTable(joinedTable);
            if (dependentTables.Count > 1)
            {
                var joinColumns = new List<ColumnExpression>();
                foreach (var property in keyProperties)
                {
                    var columnExpression = CreateColumnExpression(
                        property, dependentMainTable.FindColumn(property)!, ownedTable, newColumnsNullable);
                    propertyExpressions[property] = columnExpression;
                    joinColumns.Add(columnExpression);
                }
        ...
        IComplexProperty complexProperty)
    {
        ...
        var tableExpression = containerProjection.TableMap[complexTypeTable];
        ...
        foreach (var property in complexProperty.ComplexType.GetProperties())
        {
            ...
            propertyExpressionMap[property] = CreateColumnExpression(
                property, column, tableExpression, isComplexTypeNullable || column.IsNullable);
        }
        ...
        var newTableMap = containerProjection.TableMap.Count == 1
            ...
            : new Dictionary<ITableBase, TableExpressionBase> { [complexTypeTable] = tableExpression };
                ...
                if (joinPredicate != null)
                {
                    ...
                    if (!containsOuterReference)
                    {
                        if (limit != null || offset != null)
                        {
                            ...
                            var subqueryTable = innerSelectExpression._tables.Single();
                            var outerColumn = ((SelectExpression)innerSelectExpression.Tables[0]).GenerateOuterColumn(
                                subqueryTable, rowNumberExpression, "row");
    ...
    private SqlRemappingVisitor PushdownIntoSubqueryInternal(bool liftOrderings = true)
    {
        ...
        var subquery = new SelectExpression(
            subqueryAlias, new List<ProjectionExpression>(), _tables.ToList(), _groupBy.ToList(), _orderings.ToList(), GetAnnotations())
        {
            IsDistinct = IsDistinct,
            Predicate = Predicate,
            Having = Having,
        ...
        if (_projection.Count > 0)
        {
            ...
            foreach (var projection in projections)
            {
                ...
                var outerColumn = subquery.GenerateOuterColumn(
                    subquery, projection.Expression, projection.Alias, assignUniqueTableAlias: false);
        ...
        if (_clientProjections.Count > 0)
        {
            for (var i = 0; i < _clientProjections.Count; i++)
            {
                ...
                {
                    _clientProjections[i] = LiftEntityProjectionFromSubquery(projection, subquery);
                }
                ...
                else if (item is SqlExpression sqlExpression)
                {
                    ...
                    var outerColumn = subquery.GenerateOuterColumn(subquery, sqlExpression, alias);
        ...
        else
        {
            foreach (var (projectionMember, expression) in _projectionMapping.ToList())
            {
                ...
                if (expression is StructuralTypeProjectionExpression projection)
                {
                    _projectionMapping[projectionMember] = LiftEntityProjectionFromSubquery(projection, subquery);
                }
                ...
                else
                {
                    ...
                    var outerColumn = subquery.GenerateOuterColumn(subquery, innerColumn, projectionMember.Last?.Name);
            ...
            && !subquery.IsDistinct)
        {
            ...
            {
                projectionMap[key] = subquery.GenerateOuterColumn(subquery, key);
            }
        }
        ...
        foreach (var (column, comparer) in identifiers)
        {
            ...
            {
                outerColumn = subquery.GenerateOuterColumn(subquery, column);
            }
        ...
        foreach (var ordering in subquery._orderings)
        {
                         ...
                         || GroupBy.Contains(orderingExpression)))
            {
                _orderings.Add(
                    ordering.Update(
                        subquery.GenerateOuterColumn(subquery, orderingExpression, assignUniqueTableAlias: false)));
            }
            else
            {
                _orderings.Clear();
        ...
        var sqlRemappingVisitor = new SqlRemappingVisitor(projectionMap, subquery, subquery);
        ...
        StructuralTypeProjectionExpression LiftEntityProjectionFromSubquery(
            StructuralTypeProjectionExpression projection,
            TableExpressionBase subqueryTable)
        {
            var propertyExpressions = new Dictionary<IProperty, ColumnExpression>();
            ...
            void HandleTypeProjection(StructuralTypeProjectionExpression typeProjection)
            {
                foreach (var property in GetAllPropertiesInHierarchy(typeProjection.StructuralType))
                {
                    ...
                    var outerColumn = subquery.GenerateOuterColumn(subqueryTable, innerColumn);
                    projectionMap[innerColumn] = outerColumn;
                    propertyExpressions[property] = outerColumn;
                }
            ...
            if (projection.DiscriminatorExpression != null)
            {
                discriminatorExpression = subquery.GenerateOuterColumn(
                    subquery, projection.DiscriminatorExpression, DiscriminatorColumnAlias);
                projectionMap[projection.DiscriminatorExpression] = discriminatorExpression;
            }

            var tableMap = projection.TableMap.ToDictionary(kvp => kvp.Key, _ => (TableExpressionBase)subquery);
                    ...
                    if (boundEntityShaperExpression != null)
                    {
                        var newValueBufferExpression =
                            boundEntityShaperExpression.ValueBufferExpression is StructuralTypeProjectionExpression innerEntityProjection
                                ? (Expression)LiftEntityProjectionFromSubquery(innerEntityProjection, subquery)
        ...
        JsonQueryExpression LiftJsonQueryFromSubquery(JsonQueryExpression jsonQueryExpression)
        {
            ...
            var newJsonColumn = subquery.GenerateOuterColumn(subquery, jsonScalarExpression);
            ...
            for (var i = 0; i < keyProperties.Count; i++)
            {
                ...
                var outerColumn = subquery.GenerateOuterColumn(subquery, innerColumn);
                projectionMap[innerColumn] = outerColumn;
    ...
    private static ColumnExpression CreateColumnExpression(
        ...
        => new(
            ...
            columnModel.PropertyMappings.First(m => m.Property == property).TypeMapping,
            nullable || columnModel.IsNullable);

    private static ColumnExpression CreateColumnExpression(ProjectionExpression subqueryProjection, TableExpressionBase table)
        => new(
            subqueryProjection.Alias, table, subqueryProjection.Type, subqueryProjection.Expression.TypeMapping!,
        ...
        bool assignUniqueTableAlias = true)
    {
        ...
        return CreateColumnExpression(_projection[index], table);
    }
    ...
    private void AddTable(TableExpressionBase table)
    {
        // We unwrap here since alias are not assigned to wrapper expressions
        var unwrappedTable = table.UnwrapJoin();
        var uniqueAlias = GenerateUniqueAlias(_usedAliases, unwrappedTable.Alias!);
        unwrappedTable.Alias = uniqueAlias;

        table = (TableExpressionBase)new AliasUniquifier(_usedAliases).Visit(table);
        _tables.Add(table);
    }
    ...
    private Expression VisitChildren(ExpressionVisitor visitor, bool updateColumns)
    {
        ...
        else
        {
            ...
            if (changed)
            {
                var newSelectExpression = new SelectExpression(Alias, newProjections, newTables, newGroupBy, newOrderings, GetAnnotations())
                {
                ...
                // them; otherwise we end up with two SelectExpressions sharing the same TableReferenceExpression instance, and if that's
                // later mutated, both SelectExpressions are affected (this happened in AliasUniquifier, see #32234).
                //
                // Note that updateColumns is false only if we're already being called from ColumnTableReferenceUpdater to replace the
                // ColumnExpressions, in which case we avoid infinite recursion.
                if (tablesChanged)
                {
                    if (updateColumns)
                    {
                        newSelectExpression =
                            (SelectExpression)new ColumnTableReferenceUpdater(this, newSelectExpression).Visit(newSelectExpression);
                    }
                    else
                    {
                        for (var i = 0; i < _tables.Count; i++)
                        {
                            var (oldTable, newTable) = (_tables[i], newTables[i]);
                            if (newTable != oldTable)
                            {
                                oldTable.Reference.Table = newTable;
                            }
                        }
                    }
                }
    ...
    ///     return this expression. This method can only be used if no tables have been added or removed; it assumes that new tables
    ///     replaces old tables at the same index.
    /// </summary>
        ...
        SqlExpression? offset)
    {
        ...
        {
            throw new ArgumentException("Can't change the number of tables with Update");
        }
        ...
        var newSelectExpression = new SelectExpression(
            Alias, projections.ToList(), tables.ToList(), groupBy.ToList(), orderings.ToList(), GetAnnotations())
        {
            _projectionMapping = projectionMapping,
        ...
        // Remap the old tables to the new ones
        for (var i = 0; i < _tables.Count; i++)
        {
            var (oldTable, newTable) = (_tables[i], tables[i]);
            if (newTable != oldTable)
            {
                oldTable.Reference.Table = newTable;
            }
