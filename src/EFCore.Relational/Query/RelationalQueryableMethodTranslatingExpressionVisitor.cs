...
public class RelationalQueryableMethodTranslatingExpressionVisitor : QueryableMethodTranslatingExpressionVisitor
{
    ...
    protected virtual ShapedQueryExpression? VisitInlineQueryRoot(InlineQueryRootExpression inlineQueryRootExpression)
    {
        ...
        for (var i = 0; i < inlineQueryRootExpression.Values.Count; i++)
        {
            ...
            {
                AddTranslationErrorDetails(RelationalStrings.NonConstantOrParameterAsInExpressionValues(translatedValue.GetType().Name));
                return null;
            }

            // TODO: Think about the null semantics angle.
            // In the general case, this ValuesExpression is a table in a SelectExpression, and SqlNullabilityProcessor *apparently*
            // doesn't recurse (look/confirm). Note that for Contains specifically we have the simplification so no subquery.

            // TODO: Poor man's null semantics - we currently only support constants and parameters in SqlNullabilityProcessor, where we
            // can see if there's actually a null value or not. When we allow arbitrary expressions ()the ValuesExpression projects out a non-nullable column if we see only non-null constants
            // or nullable columns.
    ...
    protected override ShapedQueryExpression? TranslateContains(ShapedQueryExpression source, Expression item)
    {
            ...
            && projectedColumn.Table == valuesExpression)
        {
            ...
            {
                var inExpression = _sqlExpressionFactory.In(translatedItem, values, negated: false);
    ...
    protected virtual Expression ApplyInferredTypeMappings(
        ...
        => new InferredTypeMappingApplier(_sqlExpressionFactory, inferredTypeMappings).Visit(expression);
    ...
    ///         <see cref="InferredTypeMappingApplier" />. We can't do this in a single pass since untyped roots
    ...
    /// </summary>
    protected class InferredTypeMappingApplier : ExpressionVisitor
    {
        private readonly ISqlExpressionFactory _sqlExpressionFactory;
        private SelectExpression? _currentSelectExpression;
        ...
        /// <summary>
        ///     Creates a new instance of the <see cref="InferredTypeMappingApplier" /> class.
        /// </summary>
        ...
        public InferredTypeMappingApplier(
            ISqlExpressionFactory sqlExpressionFactory,
            IReadOnlyDictionary<(TableExpressionBase, string), RelationalTypeMapping?> inferredTypeMappings)
            => (_sqlExpressionFactory, _inferredTypeMappings) = (sqlExpressionFactory, inferredTypeMappings);
