...
public class RelationalQueryableMethodTranslatingExpressionVisitor : QueryableMethodTranslatingExpressionVisitor
{
    ...
    protected virtual ShapedQueryExpression? VisitInlineQueryRoot(InlineQueryRootExpression inlineQueryRootExpression)
    {
        ...
        var valuesExpression = new ValuesExpression("v", rowExpressions, new[] { "_ord", "Value" });

        // Note: we leave the element type mapping null, to allow it to get inferred based on queryable operators composed on top.
        var selectExpression = new SelectExpression(
            valuesExpression, columnName: "Value", columnType: elementType.UnwrapNullableType(), columnTypeMapping: null, isColumnNullable: encounteredNull);
        ...
        [NotNullWhen(true)] out ShapedQueryExpression? simplifiedQuery)
    {
        ...
        {
            // TODO: We want to pattern match on the projection, i.e. compare the ColumnExpression in the predicate to the
            // SelectExpression's projection, but we can't do that without applying the projection; we can't apply the projection because
            // the pattern matching may fail and we'll have wrongly changed the SelectExpression.
            ...
            if (clonedSelect.Projection is not [{ Expression: ColumnExpression { Name: var columnName}}])
            {
            ...
            if (left is ColumnExpression leftColumn
                && ReferenceEquals(leftColumn.Table, valuesExpression)
                && leftColumn.Name == columnName)
            {
                item = right;
            }
            else if (right is ColumnExpression rightColumn
                     && ReferenceEquals(rightColumn.Table, valuesExpression)
                     && rightColumn.Name == columnName)
            {
                item = left;
            }
            else
    ...
    ///     </para>
    /// </remarks>
    private class ColumnTypeMappingScanner : ExpressionVisitor
    {
    ...
    protected class RelationalInferredTypeMappingApplier : ExpressionVisitor
    {
                ...
                case ValuesExpression valuesExpression:
                    ...
                    return ApplyTypeMappingsOnValuesExpression(
                        ...
                        && !_currentSelectExpression.Projection.Any(
                            p => p.Expression is ColumnExpression { Name: "_ord" } c && c.Table == valuesExpression));
        ...
        {
            var inferredTypeMappings = InferredTypeMappings.TryGetValue((valuesExpression, "Value"), out var typeMapping)
                ? new[] { null, typeMapping }
                : new RelationalTypeMapping?[] { null, null };

            Check.DebugAssert(valuesExpression.ColumnNames[0] == "_ord", "First ValuesExpression column isn't _ord");
